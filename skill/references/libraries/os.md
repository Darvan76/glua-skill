...
| `%z` | Timezone | `-0300` |
| `%%` | A percent sign | `%` |
- `number time` = `os.time()` — Time to use for the format.
...
### `os:difftime` `[client/menu/server]`

Subtracts the second from the first value and rounds the result.

**Argumentos:**

- `number timeA` — The first value.
- `number timeB` — The value to subtract.
...
- `table dateData` = `nil` — Table to generate the time from. This table's data is interpreted as being in the local timezone. See Structures/DateData
